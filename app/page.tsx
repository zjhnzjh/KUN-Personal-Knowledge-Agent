...
type ModelCatalog = {
  embedding: Array<{ model: string; provider: string; dimensions: number[] }>;
  reranker: Array<{ model: string; provider: string; dimensions: number[] }>;
  custom_model_supported: boolean;
};
type ExperimentSweep = {
...
function TracePicker({ traces, selectedTrace, onSelect }: { traces: InfraTrace[]; selectedTrace: InfraTrace | null; onSelect: (id: string) => void }) {
  return <section className="infra-panel trace-master"><header><div><span>TRACE EXPLORER</span><h2>最近请求</h2></div><b>{traces.length} TRACES</b></header>
    <label className="trace-picker"><span>选择要分析的请求</span><select value={selectedTrace?.id || ""} onChange={(event) => { if (event.target.value) onSelect(event.target.value); }}><option value="">请选择一条 Trace…</option>{traces.map((trace) => <option key={trace.id} value={trace.id}>{trace.name.replaceAll("_", " ")} · {trace.trace_type} · {formatDate(trace.started_at)} · {trace.duration_ms == null ? "running" : `${trace.duration_ms} ms`}</option>)}</select></label>
    {selectedTrace ? <div className="trace-picker-current"><i className={selectedTrace.status} /><span><strong>{selectedTrace.name.replaceAll("_", " ")}</strong><small>{selectedTrace.trace_type} · {formatDate(selectedTrace.started_at)} · {selectedTrace.spans?.length || 0} spans</small></span><b>{selectedTrace.duration_ms == null ? "running" : `${selectedTrace.duration_ms} ms`}</b></div> : <p className="infra-empty">从下拉框选择一条请求，右侧查看阶段瀑布图。</p>}
  </section>;
}
...
function InfraView({ activeSpace, health }: { activeSpace?: Space; health: Health | null }) {
  ...
  const [sweeps, setSweeps] = useState<ExperimentSweep[]>([]);
  ...
  const [experimentRerankerTopN, setExperimentRerankerTopN] = useState(10);
  const [experimentCaseStatus, setExperimentCaseStatus] = useState<"accepted" | "exploratory">("accepted");
  ...
  async function loadInfra() {
    try {
      const [overviewData, jobData, traceData, indexData, datasetData, experimentData, sweepData, benchmarkData, policyData, budgetData, catalogData] = await Promise.all([
        ...
        api<ExperimentSweep[]>("/api/eval/sweeps?limit=20"),
        ...
        api<ModelCatalog>(`/api/eval/model-catalog?space_id=${encodeURIComponent(spaceId)}`),
      ]);
      setOverview(overviewData); setJobs(jobData); setTraces(traceData); setIndexes(indexData);
      setDatasets(datasetData); setExperiments(experimentData); setSweeps(sweepData); setBenchmarks(benchmarkData); setCloudPolicies(policyData); setInfraBudget(budgetData); setModelCatalog(catalogData);
    } catch (error) {
  ...
  async function createSweep() {
    const dataset = selectedDataset || datasets.find((item) => (item.accepted_count || 0) > 0);
    if (!dataset) { setNotice("请先选择评测数据集。"); return; }
    if (!selectedIndexId) { setNotice("Sweep 需要至少一个已就绪的向量索引；BM25 单实验不需要索引。"); return; }
    const configs = [
    ...
    await runAction("sweep", async () => {
      await api("/api/eval/sweeps", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dataset_version_id: dataset.id, name: `Retrieval matrix · ${new Date().toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}`, case_status: experimentCaseStatus, configs }) });
      setNotice("Sweep 已进入持久任务队列：串行运行、逐个保存 Run、某个失败不会丢失其他结果。");
    });
  }
  ...
  return <div className="page infra-page">
    ...
    {tab === "traces" && <div className="infra-trace-layout"><TracePicker traces={traces} selectedTrace={selectedTrace} onSelect={(id) => void openTrace(id)} /><section className="infra-panel trace-detail"><header><div><span>SPAN WATERFALL</span><h2>{selectedTrace?.name.replaceAll("_", " ") || "选择一个 Trace"}</h2></div>{selectedTrace && <b>{selectedTrace.status.toUpperCase()}</b>}</header>{selectedTrace ? <><div className="trace-summary-line"><span>ID {selectedTrace.id.slice(0, 12)}</span><span>{selectedTrace.spans?.length || 0} spans</span><span>{selectedTrace.duration_ms || 0} ms</span></div><div className="span-waterfall">{(selectedTrace.spans || []).map((span) => <article key={span.id}><span><strong>{span.operation}</strong><small>{span.kind}</small></span><div><i className={span.status} style={{ width: `${Math.max(3, Number(span.duration_ms || 0) / maxSpan * 100)}%` }} /></div><b>{span.duration_ms || 0} ms</b></article>)}</div><details><summary>Trace attributes</summary><pre>{JSON.stringify(selectedTrace.attributes, null, 2)}</pre></details></> : <p className="infra-empty">从下拉框选择请求，查看 BM25、FAISS、RRF、Rerank、LLM 与缓存阶段的真实耗时。</p>}</section></div>}
